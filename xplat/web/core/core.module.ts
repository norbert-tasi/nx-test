...
import { throwIfAlreadyLoaded } from '@nx-test/utils';
import {
  CoreModule,
...
export class NxTestCoreModule {
  constructor(
    @Optional()
    @SkipSelf()
    parentModule: NxTestCoreModule
  ) {
    throwIfAlreadyLoaded(parentModule, 'NxTestCoreModule');
  }
}
